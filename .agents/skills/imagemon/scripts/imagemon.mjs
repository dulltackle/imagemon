...
var package_default = {
  ...
  scripts: {
    ...
    "mobile:lint": "npm run lint --workspace @imagemon/mobile",
    "mobile:test": "npm run test --workspace @imagemon/mobile",
    "mobile:typecheck": "npm run typecheck --workspace @imagemon/mobile",
    "mobile:verify": "npm run mobile:lint && npm run mobile:typecheck && npm run mobile:test && npm run check:mobile-icons",
    ...
    verify: "npm run build && npm run typecheck && npm run check:versions && npm run check:adrs && npm run test:coverage && npm run check:skill && npm run check:promptdex && npm run check:skills",
...
function loadImageConfig(configPathOption) {
...
function wrapFetch(customFetch) {
  if (!customFetch) {
    return void 0;
  }
...
async function generateImage(options, clientOptions = {}) {
  ...
  if (options.stream) {
    const stream = await client.images.generate(body);
    return normalizeImageStream(stream);
  }
...
async function editImage(options, clientOptions = {}) {
  ...
  if (options.stream) {
    const stream = await client.images.edit(body);
    return normalizeImageStream(stream);
  }
...
function normalizeImageResponse(response) {
  ...
  const images = typedResponse.data?.map((image) => {
...
async function* normalizeImageStream(stream) {
  for await (const event of stream) {
...
function normalizeStreamEvent(event) {
  ...
  if (typedEvent.type !== "image_generation.partial_image" && typedEvent.type !== "image_generation.completed" && typedEvent.type !== "image_edit.partial_image" && typedEvent.type !== "image_edit.completed") {
    return null;
  }
  ...
  try {
    ...
    for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount += 1) {
      ...
      try {
        ...
        if (error === timeoutError || isAbortError2(error)) {
          throw timeoutError;
        }
...
function isPrivateAddress(address) {
  ...
  return first === 0 || first === 10 || first === 127 || first === 169 && second === 254 || first === 172 && second >= 16 && second <= 31 || first === 192 && second === 168 || first === 100 && second >= 64 && second <= 127 || first === 192 && second === 0 && third === 0 || first === 192 && second === 0 && third === 2 || first === 198 && (second === 18 || second === 19) || first === 198 && second === 51 && third === 100 || first === 203 && second === 0 && third === 113 || first >= 224;
}
function isPrivateIpv6(address) {
  ...
  return firstGroup < 8192 || firstGroup > 16383;
}
...
function validateContentType(response, allowedContentTypes) {
...
async function withTimeout(operation, timeoutPromise) {
  return await Promise.race([operation, timeoutPromise]);
}

// src/lib/image-output.ts
var DEFAULT_OUTPUT_DIR = "outputs";
var DEFAULT_OUTPUT_FORMAT = "png";
var GENERATED_BASE_NAME_ATTEMPTS = 5;
var TEMP_DIRECTORY_PREFIX = ".imagemon-";
...
async function prepareImageOutputDirectory(outDir) {
...
function normalizeOutputFormat(responseFormat, requestedFormat) {
...
function isOutputFormat(value) {
  return value !== void 0 && OUTPUT_FORMATS.has(value);
}
...
async function writeImageResult(result, outDir, baseName, outputFormat, createdAt, options) {
  ...
  try {
    for (const [index, image] of result.images.entries()) {
      ...
      stagedFiles.push({ tempPath, path: path2 });
    }
    const metadata = {
...
function outputExistsError(error) {
  const path2 = typeof error.path === "string" ? error.path : "unknown output path";
  return new Error(`Output file already exists: ${path2}. Set overwrite: true to replace it.`);
}
async function imageBytes(image, downloadOptions) {
  if (image.b64_json) {
    return Buffer2.from(image.b64_json, "base64");
  }
  if (image.url) {
...
async function commitStagedFiles(stagedFiles, tempDir, overwrite) {
  const reservedPaths = [];
  const backupFiles = [];
  ...
  try {
    if (overwrite) {
      for (const [index, stagedFile] of stagedFiles.entries()) {
        const backupPath = join2(tempDir, `backup-${index}`);
...
async function rollbackCommit(committedPaths, reservedPaths, backupFiles) {
  const errors = [];
  const createdPaths = [.../* @__PURE__ */ new Set([...committedPaths, ...reservedPaths])];
  for (const path2 of createdPaths.reverse()) {
    try {
      await rm(path2, { force: true });
    } catch (error) {
      errors.push(`failed to remove ${path2}: ${errorMessage(error)}`);
    }
  }
...
`;
var OUTPUT_FORMATS2 = /* @__PURE__ */ new Set(["png", "jpeg", "webp"]);
...
async function runImagemonCli(argv, options = {}) {
  ...
  try {
    ...
    const saved = await saveImageResult(result, {
      ...
      request: {
        ...
        n: parsed.n,
        image: parsed.imagePath ? resolve3(parsed.imagePath) : void 0,
        mask: parsed.maskPath ? resolve3(parsed.maskPath) : void 0
      }
    });
...
function buildGenerateOptions(parsed) {
...
async function buildEditOptions(parsed) {
  if (!parsed.imagePath) {
    throw new Error("--image is required for edit");
  }
  const image = await uploadableFromPath(parsed.imagePath);
  const mask = parsed.maskPath ? await uploadableFromPath(parsed.maskPath) : void 0;
  return stripUndefined({
    model: parsed.model,
    image,
    mask,
...
async function uploadableFromPath(path2) {
  return toFile(createReadStream(path2), basename(path2));
}
function parseOutputFormat(value) {
  if (value === void 0) {
    return void 0;
  }
  if (!OUTPUT_FORMATS2.has(value)) {
...
function parseIntegerOption(values, name) {
...
`);
  ...
  if (argv.length === 1 && (argv[0] === "--help" || argv[0] === "help") || argv.length === 2 && (argv[0] === "generate" || argv[0] === "edit") && argv[1] === "--help") {
