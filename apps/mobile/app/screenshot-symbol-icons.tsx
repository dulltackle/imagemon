...
export default function ScreenshotSymbolIconsScreen() {
  ...
  if (page === null) {
    return (
      <View className="flex-1 items-center justify-center gap-3 bg-app-surface-raised p-6">
        <Text
  ...
  return (
    <View className="flex-1 gap-4 bg-app-surface-raised p-4">
      <View
      ...
      <View className="flex-1 flex-row flex-wrap content-start justify-between gap-y-3">
        {pageIconNames.map((name) => (
          <View
            className="items-center gap-2 rounded-lg bg-app-surface p-3"
            key={name}
            style={{ width: "48%" }}
          >
            <View
