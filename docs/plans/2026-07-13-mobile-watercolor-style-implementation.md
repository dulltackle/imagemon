...
- **基线**：当前 worktree 为 `v0.10.0`，HEAD `1d3e511`，处于 detached HEAD。
- **当前未提交前置改动**：ADR 0063 已被 ADR 0210 取代，Expo 和 Web 已切换为固定浅色，并加入 `expo-system-ui`。这些改动必须先独立提交，不能与水彩视觉实现混为一个提交。
...
- `PromptdexEntryDetailScreen.tsx` 仍有 `bg-blue-50` 主题逃逸点；
...
1. 从当前 detached HEAD 创建或切换到 `codex/mobile-watercolor-style`（若已有承载该工作的分支，则使用既有分支）；
2. 先把当前固定浅色模式改动整理成独立提交；
3. 使用 Image Gen 生成 `catalog-loaded-mobile.png`、`catalog-empty-mobile.png`、`entry-workbench-mobile.png` 和 `catalog-tablet.png`；
4. 由开发者确认视觉方向；
5. 根据获批概念稿冻结 token 与生产资产清单；
6. 新增 ADR 0211；
7. 开始“主题与媒体基础”提交；
8. 完成图鉴首页试点并进行概念图对照验收。
