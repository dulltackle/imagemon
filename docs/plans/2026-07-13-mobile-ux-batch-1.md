...
### 明确不做（并说明理由）
**ADR 0039 的「图片规格 × 模型能力兼容校验」不在本批。** 0039 的触发条件是「选择或**切换**图片模型配置后」，而当前表单**不提供模型切换**（只读展示默认图片模型配置，`renderModelConfiguration()` 609 行）。校验的前提是 ADR 0221 的「本地图片模型能力档案」先落地（`@imagemon/core` 里有 `IMAGE_MODEL_CAPABILITIES`，但移动端未接）。这两件事一起做才有意义，单列 roadmap。
...
- **明确留下**：1.9（输入说明文案出现两遍）留给第三批的文案统一；ADR 0039 的规格 × 模型能力兼容校验留给「模型能力档案（ADR 0221）落地」时一起做；`format` / `n` 的可编辑化留给后续（需要动 `image_results` 的 CHECK 约束与多结果模型）。
